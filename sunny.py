...
if options['clientid'] == '':
    if not python_version:
        input_clientid = 181714146453
    else:
        input_clientid = str(181714146453)
    if input_clientid != '':
                    ...
                    if type == 1:
                        ...
                        if js['Type'] == 'NewTunnel':
                            if js['Payload']['Error'] != '':
                                ...
                                print('隧道建立失败时间如下: %s' % js['Payload']['Error'])
                                print(time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())))
                                time.sleep(30)
                            else:
                                logger = logging.getLogger('%s' % 'client')
                                logger.debug('Tunnel established at %s' % js['Payload']['Url'])
                                print('隧道建立成功时间如下: %s' % js['Payload']['Url']) # 注册成功
                                print(time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())))
                    if type == 2:
